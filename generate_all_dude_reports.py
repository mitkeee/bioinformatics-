#!/usr/bin/env python3
"""
Generate comprehensive detailed reports for all DUDE proteins
Uses PDB files directly from dude_1_2 folder
"""
...
def generate_report_for_protein(pdb_file, protein_id, output_dir, params):
    ...
    try:
        ...
        # Try to extract DSSP and STRIDE (may be empty)
...
def write_summary_statistics(f, df, params):
    ...
    if dssp_mask.sum() > 0:
        ...
        f.write(f"  - DSSP Cutoff Value: ASA ≥ {params.dssp_asa_cutoff}%\n\n")
    else:
        f.write("DSSP Classification:\n")
    ...
    if stride_mask.sum() > 0:
        ...
        f.write(f"  - STRIDE Cutoff Value: ASA ≥ {params.stride_asa_cutoff}%\n\n")
    else:
        f.write("STRIDE Classification:\n")
...
def write_statistics_section(f, df):
    ...
    if dssp_mask.sum() > 0:
        ...
        write_confusion_matrix(f, cm, acc, prec, rec, f1)
        f.write("\n")
    else:
        f.write("ACCORDING TO DSSP (Ground Truth = DSSP Classifications):\n")
        f.write("=" * 100 + "\n\n")
...
def write_confusion_matrix(f, cm, acc, prec, rec, f1):
...
def write_legend(f, params):
    ...
    f.write("ID        : Residue amino acid code\n")
    ...
    f.write("DSSP SS   : DSSP secondary structure\n")
    ...
    f.write("NC6/NC10  : Neighbor counts within 6Å/10Å spheres\n")
    f.write("Uni6/Uni10: Uniformity at 6Å/10Å\n")
...
def main():
    ...
    print("\n" + "=" * 100)
    print("GENERATING COMPREHENSIVE DETAILED REPORTS FOR ALL DUDE PROTEINS")
    print("=" * 100 + "\n")
